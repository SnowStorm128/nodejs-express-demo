...
const logger = winston.createLogger({
    format: winston.format.combine(winston.format.label({ label: '12' }), winston.format.timestamp(), winston.format.json()),
...
catRouter.get('/', async (req, res) => {
    ...
    //     res.end(message);
    // })
    const worker1 = await runWorker({ number: 12 }).then((data) => data.result);
    res.end(worker1);
});
